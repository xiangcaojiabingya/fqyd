  ...
  <div>
    ...
    <div class="history-div" @click="onClick">
      <div class="fixfloat history-top">
        <div class="history-title">{{ item.productName }}-{{ getID(item) }}</div>
        <div class="history-time">{{ dealTime(item.createTime) }}</div>
      </div>
      <div class="fixfloat history-bottom">
        <div class="history-sum">{{title}} {{getMoney(item.approvalAmount) }}</div>
        <div
          class="history-status"
          :style="{ color: funSetColor(item) }"
        >{{ getZHType(item.status) }}</div>
      </div>
    </div>
  </div>
</template>
...
<script>
import ItemCuttingLine from "./ItemCuttingLine.vue";
